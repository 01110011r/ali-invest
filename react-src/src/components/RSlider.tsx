...
console.log(children[0])
    ...
    const slideHandler= () => React.Children.map(children, (child) => {
    ...
    return (
        ...
        <div className={'relative transition-all ease-in-out duration-500 overflow-hidden'}>
            ...
            <div className={'mb-4'}>
                {
                    children.map((child, idx) => (<span className={idx===conf.current-1 ? "bg-[#F97B07] inline-block w-1/4 h-2 rounded-lg transition-all ease-in-out duration-100 transform" : "bg-[#EDEDED] rounded-l rounded-r inline-block w-1/4 h-2 transition-all ease-in-out duration-100 transform"}></span>))
